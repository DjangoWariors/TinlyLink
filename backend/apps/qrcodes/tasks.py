...
def track_qr_scan(scan_data):
    ...
    try:
        ...
        if hasattr(qr, "serial"):
            ...
            # Increment serial scan count
            serial.total_scans = models.F("total_scans") + 1
            serial.save(update_fields=["total_scans"])

        # Create unified click event
        ClickEvent.objects.create(
            # Link reference (may be null for non-link QRs)
...
def generate_serial_batch(self, batch_id):
    ...
    Runs asynchronously for large batches with progress tracking.

    Creates QR codes with unique serial numbers, generates images,
    packages into ZIP file with CSV manifest.
    """
    import csv
    ...
    try:
        ...
        # Prepare CSV data
        csv_rows = [
            ["serial_number", "qr_code_id", "short_code", "verify_url", "destination_url"]
        ]

        # Determine base URL for verification
        base_url = getattr(settings, "SITE_URL", f"https://{settings.DEFAULT_SHORT_DOMAIN}")

        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for i in range(batch.quantity):
                # Generate unique serial number
                serial_suffix = uuid_module.uuid4().hex[:12].upper()
                serial_number = f"{batch.prefix}{serial_suffix}" if batch.prefix else serial_suffix

                # Ensure uniqueness
                while SerialCode.objects.filter(serial_number=serial_number).exists():
                    serial_suffix = uuid_module.uuid4().hex[:12].upper()
                    serial_number = f"{batch.prefix}{serial_suffix}" if batch.prefix else serial_suffix

                # Build destination URL from template
                destination_url = batch.destination_url_template.replace(
                ...
                qr = QRCode.objects.create(
                    ...
                    team=batch.team,
                    qr_type="serial",
                    title=f"{batch.product_name} - {serial_number}" if batch.product_name else serial_number,
                    is_dynamic=True,
                ...
                # Create serial code record
                serial_code = SerialCode.objects.create(
                    batch=batch,
                    qr_code=qr,
                    serial_number=serial_number,
                    metadata={
                ...
                verify_url = f"{base_url}/verify/{serial_number}"

                # Generate QR image with verification URL as content
                try:
                    ...
                    zf.writestr(f"qr-codes/{serial_number}.png", img_buffer.getvalue())

                except Exception as img_error:
                    logger.warning(f"Failed to generate image for {serial_number}: {img_error}")
                ...
                # Update progress
                batch.generated_count = i + 1
                batch.save(update_fields=["generated_count"])

                # Update Celery task state for progress tracking
                if (i + 1) % 10 == 0 or i == batch.quantity - 1:
                    self.update_state(
                        state="PROGRESS",
                        meta={
                            "current": i + 1,
                            "total": batch.quantity,
                            "percent": round(((i + 1) / batch.quantity) * 100, 1),
                        }
                    )
            ...
            zf.writestr("serial_codes.csv", csv_buffer.getvalue())
        ...
        if settings.DEBUG:
            ...
            full_path.parent.mkdir(parents=True, exist_ok=True)
            full_path.write_bytes(zip_buffer.read())
        ...
        batch.save(update_fields=["status", "export_file_url", "completed_at"])
