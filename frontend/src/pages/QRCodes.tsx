...
// Skeleton for QR card loading state
...
export function QRCodesPage() {
  ...
  const handleDownload = async (qr: QRCodeType, format: 'png' | 'svg') => {
    try {
      // Use client-side generation to preserve frames/styles
      const element = document.getElementById(`qr-generator-${qr.id}`);
      if (!element) {
      ...
      link.href = dataUrl;
      link.download = `qr-${qr.link_short_code}.${format}`;
  ...
  // Render all current page QRs in hidden container for download availability
  const visibleQRs = qrCodes;
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="bg-primary-50 border-primary/20">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setSelectedQRs(new Set())} className="text-sm text-gray-500 hover:text-gray-700">
                Clear
              </button>
            </div>
      ...
      {/* Search and Filters - Same as LinksPage */}
      <Card>
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            Filter
            {hasActiveFilters && (
              <span className="ml-1.5 w-2 h-2 bg-primary rounded-full" />
            )}
          </Button>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  ...
                  value={styleFilter}
                  onChange={(e) => {
                    setStyleFilter(e.target.value as QRStyle | '');
                    setPage(1);
                  }}
                >
          ...
          {qrCodes.map((qr: QRCodeType) => (
            <Card key={qr.id} padding="none" className={`group relative ${selectedQRs.has(qr.id) ? 'ring-2 ring-primary' : ''}`}>
              ...
              <div
                ...
                style={{ backgroundColor: qr.background_color }}
              >
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => window.open(qr.short_url, '_blank')}
                  >
      ...
      {/* Pagination - Same style as LinksPage */}
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 order-1 sm:order-2">
            ...
            <span className="px-3 py-1.5 text-sm text-gray-600 min-w-[90px] text-center">
          ...
          {qrToDelete && (
            <div className="p-4 bg-gray-50 rounded-lg flex items-center gap-4">
              <div
                ...
                style={{ backgroundColor: qrToDelete.background_color }}
              >
                <QRCodeGenerator
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-gray-900 truncate">
                  {qrToDelete.link_short_code}
                </p>
                <p className="text-xs text-gray-500 truncate mt-0.5">
                  {qrToDelete.link_original_url}
                </p>
                {qrToDelete.total_scans > 0 && (
                  <p className="text-xs text-gray-400 mt-1">
                    {qrToDelete.total_scans} scan{qrToDelete.total_scans !== 1 ? 's' : ''}
                  </p>
                )}
              </div>
            </div>
